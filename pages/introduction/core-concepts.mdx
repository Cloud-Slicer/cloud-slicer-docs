...
<Stepper>
    ...
    4. ## Quote Operations
        ...
        - **Quote File:** `POST /quote/fdm/{file_id}` - Queue an async quote on an uploaded file given its **file_id**.  Returns a **quote_id** to poll on.
        - **Get Quote Record:** `GET /quote/{quote_id}` - Retrieve a quote record given its **quote_id** you received when creating the quote on the `/quote/fdm/{file_id}` route.
        - **Delete Quote:** `DELETE /quote/{quote_id}` — Delete a quote record along with its G-code file per its **quote_id**
        - **Download .gcode:** `GET /quote/output/{quote_id}` — Download the generated G-code file for the quote per its **quote_id**
