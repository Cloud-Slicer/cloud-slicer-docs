...
const response = await fetch(`${BASE_URL}/quote/fdm/${fileId}`, {
  method: "POST",
  headers: {
...
- **`filament_breakdown`** — per-feature material usage: grams and percent for
  perimeters, infill, supports and the rest, plus `support_percent` and
  `support_weight_g`. Reporting only — it never changes the price, and it's `null`
  for SLA quotes and for any quote whose g-code couldn't be walked
...
response = requests.get(
    f"{BASE_URL}/quote/output/{quote_id}",
    headers={"Authorization": f"Bearer {API_TOKEN}"},
    stream=True,
)
response.raise_for_status()
...
const response = await fetch(`${BASE_URL}/quote/output/${quote_id}`, {
  headers: { Authorization: `Bearer ${API_TOKEN}` },
});
