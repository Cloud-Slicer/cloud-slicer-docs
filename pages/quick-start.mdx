...
- **[In the API Playground](https://api.cloudslicer3d.com/docs)**:
   1. Authenticate with the API in our [Swagger Docs playground](https://api.cloudslicer3d.com/docs)
   2. Make your first API call to ```/v1/file``` to upload a file
   3. Make an API call to ```/v1/quote/{file-id}``` using your `file_id`, `printer_id`, and `filament_id` to get a quote for your uploaded file
...
6. ## Make Your First Set of API Calls ##
   ...
   **Create your first quote:**
   ...
   1. **Find** the `POST /v1/quote/{file-id}` endpoint in the API documentation
   2. **Expand** the tab to reveal the POST request details
   3. **Add Details**:
      - Fil in the path parameter with the `file_id` from the previous upload step
      - Fill in both the `printer_id` and `filament_id` fields in the request body with the IDs created in Steps 3 and 4
      - Fill in the print settings for your quote
   4. **Execute** the request
   5. **Copy** the file_id from the response
